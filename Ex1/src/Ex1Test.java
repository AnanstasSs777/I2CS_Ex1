...
class Ex1Test {
	...
	public void testArea2() {
		...
		double area =2.5;
		assertEquals(a1,area, Ex1.EPS); // area, a1 (switch) it doesn't correct
		assertEquals(a2,area, Ex1.EPS); // area, a2 (switch)
		assertEquals(a3,area, Ex1.EPS); // area, a3 (switch)
		assertEquals(a100,area, Ex1.EPS); // area, a100 (switch)
	}
	@Test
	/**
    ...
    @Test
    public void testPolynomFrom1Point() {
        double[] xx = {3};
        double[] yy = {7};
    ...
    @Test
    public void testPolynomFrom0Points() {
        double[] xx = {};
        double[] yy = {};
        double[] p = Ex1.PolynomFromPoints(xx, yy);
        assertNull(p);
    ...
    public void testPolynomFrom2Points() {
        ...
        double[] yy = {5, 1};
        double[] p = Ex1.PolynomFromPoints(xx, yy);
        double[] expected = {11, -2, 0};
        assertNotNull(p);
        assertArrayEquals(p, expected);
    }
    ...
    public void testPolynomFrom3Points() {
        ...
        double[] yy = {5, 1, 9};
        double[] p = Ex1.PolynomFromPoints(xx, yy);
        double[] expected = {-79, 46, -6};
        assertNotNull(p);
        assertArrayEquals(p, expected);
    }

    @Test
    public void testPolynomFrom4Points() {
        double[] xx = {3, 5, 4, 7};
        double[] yy = {5, 1, 9, 2};
        double[] p = Ex1.PolynomFromPoints(xx, yy);
        assertNull(p);
    ...
    public void testLength1() {
        assertEquals(10, Ex1.length(new double[]{5}, 0, 10, 1));
        assertEquals(10, Ex1.length(new double[]{5}, 0, 10, 1000));
        assertEquals(Math.sqrt(18), Ex1.length(new double[]{0, 1}, 0, 3, 1000), Ex1.EPS);
        assertEquals(1.47894, Ex1.length(new double[]{0, 0, 1}, 0, 1, 5000), Ex1.EPS);
    }
}
